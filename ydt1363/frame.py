...
from .utils import to_ascii_hex_bytes, calculate_lchksum, calculate_chksum, SOI, EOI
import binascii
...
class RealtimeDataFrame:
    ...
    def __init__(self, data: bytes):
        ...
        self.cell_voltages = [int.from_bytes(data[18+i*2:20+i*2], 'big') / 1000 for i in range(self.number_of_cells)]
        index = 18 + self.number_of_cells * 2
        self.cell_temperature = data[index]
        index += 1
        self.battery_temperature = [int.from_bytes(data[index + i*2:index + i*2 + 2], 'big') / 10 for i in range(4)]
        index += 8
        ...
        self.battery_status = BatteryStatus((int.from_bytes(data[index:index+2], 'big') & 0x7000) >> 12)
        index += 2
        ...
        self.current_limit = CurrentLimit((int.from_bytes(data[index:index+2], 'big') & 0x00F0) >> 4 )
        index += 2
    ...
    def __str__(self) -> str:
        return (f"RealtimeDataFrame(SLAVE_ADDR={self.slave_addr:02X}, "
    ...
    def serialize(self) -> bytes:
        ...
        current_state_val = (int(self.current_state) & 0xAFFF) | ( (self.battery_status.value & 0x7) << 12)
...
class InfoType:
    ...
    def serialize(self) -> bytes:
        if isinstance(self.info, bytes):
            ...
            raise TypeError(f"Unexpected type for info, found {type(self.info)} expected bytes or RealtimeDataFrame")
        ...
        if self.cid2 == 0x85:
            ...
            if isinstance(info.info, bytes):
                ...
                if not isinstance(info.info, RealtimeDataFrame):
                    raise TypeError(f"Unexpected type for info, found {type(info.info)} expected bytes of RealtimeDataFrame")
        else:
            self.info = info
